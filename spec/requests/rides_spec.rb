...
RSpec.describe 'Rides API', type: :request do
  ...
  describe 'POST /rides' do
    ...
    context 'when the request is valid' do
      before { post '/rides', params: valid_attributes }
    ...
    context 'when the request is invalid' do
      before { post '/rides', params: { origin: 'Hamilton' } }
      ...
      it 'returns a validation failure message' do
        expect(response.body)
            .to match(/Validation failed: Destination can't be blank, Leave at can't be blank, Arrive at can't be blank, Price can't be blank, Seats can't be blank/)
      end
    end
  end
  ...
  describe 'PUT /rides/:id' do
    let(:valid_attributes) { { origin: 'Barrie' } }

    context 'when the record exists' do
      before { put "/rides/#{ride_id}", params: valid_attributes }
